<template>
  <div class="proxy-page">
    <div class="card">
      ...
      <template v-if="!isMobile">
        <div class="setting-row flex-between">
          <div>
            <div>🛡️ TUN 模式</div>
            <div class="info-item">接管全部网络流量（含不遵守系统代理的 AI 软件、游戏等）</div>
          </div>
          <van-switch v-model="tunEnabled" size="22px" active-color="#e74c3c" @change="onTunModeChange" />
        </div>
        <div v-if="tunEnabled" class="mode-hint-tun">
          <div v-if="!isAdmin" class="scope-warning">⚠ TUN 模式需要管理员权限，请以管理员身份运行本程序</div>
          <div class="proxy-scope-row" style="margin-top: 6px">
            <span class="scope-label">TUN 栈:</span>
            <van-radio-group v-model="tunStack" direction="horizontal">
              <van-radio name="gvisor" checked-color="#e74c3c">gvisor（免驱动）</van-radio>
              <van-radio name="system" checked-color="#e74c3c">system（需wintun）</van-radio>
            </van-radio-group>
          </div>
        </div>
        ...
        <div class="setting-row">
          <div class="proxy-scope-row">
            ...
            <van-radio-group v-model="proxyRange" direction="horizontal">
              <van-radio name="all" checked-color="#e74c3c">全局</van-radio>
              <van-radio name="foreign" checked-color="#e74c3c">绕过境内（仅代理境外）</van-radio>
            </van-radio-group>
          </div>
          <div v-if="proxyRange === 'foreign'" class="info-item" style="margin-top: 4px">
            国内流量直连，境外流量走代理（自动注入 GEOIP,CN,DIRECT 规则）
          </div>
          <div v-else class="info-item" style="margin-top: 4px">
            所有流量均走代理
          </div>
        </div>
        <template v-if="!tunEnabled">
          <div class="setting-divider"></div>
          <div class="setting-row flex-between">
            <div>
              <div>🌐 浏览器代理</div>
          ...
          <div class="setting-row">
            <div class="proxy-scope-row">
              <span class="scope-label">浏览器范围:</span>
    ...
    <div class="card">
      <div class="card-title-group">
        <span class="card-title">🛡️ 高级设置</span>
        <button class="btn-help" @click="showHelp('advanced')">?</button>
      </div>
      <template v-if="!isMobile">
        <div class="setting-row">
          <div class="proxy-scope-row">
            <span class="scope-label">TLS 指纹伪装:</span>
            <select class="dark-select fingerprint-select" v-model="tlsFingerprint">
          ...
          <div class="info-item">伪装 TLS 指纹为浏览器特征，防止流量被识别</div>
        </div>
        <div class="setting-divider"></div>
        <div class="setting-row flex-between">
          <div>
            <div>🔍 域名嗅探</div>
            <div class="info-item">从 IP 流量还原域名，让代理规则更精准</div>
          </div>
          <van-switch v-model="sniffingEnabled" size="22px" active-color="#e74c3c" @change="onAdvancedChange" />
        </div>
        <div class="restart-hint">
          ⚠ 修改高级设置后需重启服务生效
        </div>
      ...
      <template v-else>
        <div class="info-item">高级设置仅在桌面版可用</div>
      </template>
    </div>
...
const proxyRange = ref('all')
const tlsFingerprint = ref('none')
const sniffingEnabled = ref(false)
const isAdmin = ref(false)
const customAppsEnabled = ref(false)
const proxyMode = ref('global')  // 手机版用：global | per_app
  ...
  try {
    if (isMobile) {
      ...
      await proxyApi.saveConfig({
        ...
        proxy_mode: tunEnabled.value ? 'tun' : 'system',
        proxy_range: proxyRange.value,
        tun_stack: tunStack.value,
        tls_fingerprint: tlsFingerprint.value,
        sniffing_enabled: sniffingEnabled.value,
      })
    }
  } catch (e) {
    console.error(e)
...
async function onTunModeChange() {
  showRestartHint.value = true
  await saveProxyConfig()
}
...
async function loadConfig() {
  try {
    if (isMobile) {
      ...
      tunEnabled.value = (data.proxy_mode || 'system') === 'tun'
      tunStack.value = data.tun_stack || 'gvisor'
      proxyRange.value = data.proxy_range || 'all'
      tlsFingerprint.value = data.tls_fingerprint || 'none'
      sniffingEnabled.value = data.sniffing_enabled || false
